...
class ToolRunnerServiceClass {
  ...
  private getDefaultParams(toolName: string): ToolParam[] {
    ...
    if (toolName === 'gobuster') {
      common.push(
        { name: 'mode', label: 'Mode', type: 'select', required: true, defaultValue: 'dir', options: [
          { label: 'Directory', value: 'dir' },
          { label: 'DNS', value: 'dns' },
          { label: 'VHost', value: 'vhost' },
        ]},
        { name: 'wordlist', label: 'Wordlist', type: 'text', required: false, placeholder: '/path/to/wordlist.txt' },
      )
    }
    if (toolName === 'nmap') {
      common.push(
        { name: 'ports', label: 'Ports', type: 'text', required: false, defaultValue: '1-1000', placeholder: '80,443,8080' },
        { name: 'scan_type', label: 'Scan Type', type: 'select', required: false, defaultValue: 'tcp', options: [
    ...
    if (toolName === 'ffuf') {
      common.push(
        { name: 'wordlist', label: 'Wordlist', type: 'text', required: false, placeholder: '/path/to/wordlist' },
        { name: 'extension', label: 'Extension', type: 'text', required: false, placeholder: 'php,asp,html' },
      )
    }
    return common
  }
  ...
  async startTool(toolName: string, params: Record<string, any>): Promise<ToolRun> {
    const run: ToolRun = {
      ...
      logs: [{ timestamp: new Date().toISOString(), level: 'info', message: `Queued ${toolName} scan...` }],
    }
    ...
    setTimeout(() => this.updateRun(run.id, { state: 'running', currentTask: 'Initializing...' }), 1000)
