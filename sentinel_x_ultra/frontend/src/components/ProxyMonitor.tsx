      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <StatusIndicator status={stats.running ? 'online' : 'offline'} label={stats.running ? 'Running' : 'Stopped'} size="md" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
            <div style={{ textAlign: 'center', padding: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize: '16px', fontWeight: '700', color: '#00d4ff' }}>{stats.port}</div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
              <div style={{ fontSize: '10px', color: '#666', marginBottom: '4px' }}>Captured</div>
              <div style={{ fontSize: '16px', fontWeight: '700', color: '#00ff88' }}>{stats.requestsCaptured}</div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
              <div style={{ fontSize: '10px', color: '#666', marginBottom: '4px' }}>RPM</div>
              <div style={{ fontSize: '16px', fontWeight: '700', color: '#ffaa00' }}>{stats.requestsPerMinute}</div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
        ...
        >
          {filteredRequests.length === 0 ? (
            <div style={{ padding: '60px 20px', textAlign: 'center', color: '#666' }}>
              ...
              <div style={{ fontSize: '14px', fontWeight: '600', marginBottom: '4px', color: '#888' }}>No requests captured</div>
              <div style={{ fontSize: '12px' }}>Configure your browser to use this proxy to capture traffic</div>
            </div>
          ) : (
              ...
              {filteredRequests.map((req) => (
                <div
                  ...
                  style={{
                    ...
                    background: selectedRequest?.id === req.id ? 'rgba(0,212,255,0.08)' : 'transparent',
                  }}
                  onMouseEnter={(e) => { if (selectedRequest?.id !== req.id) e.currentTarget.style.background = 'rgba(255,255,255,0.03)' }}
                  onMouseLeave={(e) => { if (selectedRequest?.id !== req.id) e.currentTarget.style.background = 'transparent' }}
                >
                  <span style={{ color: methodColors[req.method] || '#888', fontWeight: '600' }}>{req.method}</span>
                  <span style={{ color: '#888', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{req.host}</span>
                  <span style={{ color: '#aaa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'monospace', fontSize: '11px' }}>{req.path}</span>
                  <span style={{
                    color: req.status < 300 ? '#00ff88' : req.status < 400 ? '#00d4ff' : req.status < 500 ? '#ffaa00' : '#ff4444',
                    fontWeight: '600',
                  }}>
                    {req.status}
                  </span>
        ...
        {selectedRequest ? (
          <>
              ...
              {(['request', 'response'] as const).map((tab) => (
                <button
                  ...
                  style={{
                    ...
                    background: 'transparent',
                    border: 'none',
                    borderBottom: inspectTab === tab ? '2px solid #00d4ff' : '2px solid transparent',
            ...
            <div style={{
              ...
              <span style={{ color: methodColors[selectedRequest.method] || '#888', fontWeight: '700' }}>{selectedRequest.method}</span>
              <span style={{ color: '#888' }}>{selectedRequest.host}</span>
              <span style={{ color: '#aaa', fontFamily: 'monospace' }}>{selectedRequest.path}</span>
              <span style={{ color: selectedRequest.status < 300 ? '#00ff88' : '#ff4444', fontWeight: '600', marginLeft: 'auto' }}>
                {selectedRequest.status}
              </span>
            </div>

            {/* Content */}
            <div style={{ flex: 1, overflow: 'auto', padding: '16px', fontFamily: 'monospace', fontSize: '12px' }}>
              {inspectTab === 'request' ? (
                <div>
                  {/* Headers */}
                  <div style={{ marginBottom: '16px' }}>
                    <div style={{ color: '#00d4ff', fontWeight: '600', marginBottom: '8px', fontSize: '11px', textTransform: 'uppercase' }}>
                      Headers
                    </div>
                  ...
                  {selectedRequest.requestCookies && (
                    <div style={{ marginBottom: '16px' }}>
                      <div style={{ color: '#ff8844', fontWeight: '600', marginBottom: '8px', fontSize: '11px', textTransform: 'uppercase' }}>
                        Cookies
                      </div>
                      <div style={{ color: '#aaa', background: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '6px' }}>
                        {selectedRequest.requestCookies}
                      </div>
                    </div>
                  )}
                  ...
                  {selectedRequest.requestBody && (
                    <div>
                      <div style={{ color: '#00ff88', fontWeight: '600', marginBottom: '8px', fontSize: '11px', textTransform: 'uppercase' }}>
                        Body
                      </div>
                      <div style={{ color: '#aaa', background: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '6px', whiteSpace: 'pre-wrap', maxHeight: '300px', overflow: 'auto' }}>
                        {selectedRequest.requestBody}
                      </div>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    <div style={{ color: '#00d4ff', fontWeight: '600', marginBottom: '8px', fontSize: '11px', textTransform: 'uppercase' }}>
                      Status: {selectedRequest.status} • Length: {selectedRequest.responseLength || selectedRequest.size} bytes
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div style={{
                        ...
                        {selectedRequest.responseBody.length > 5000
                          ? selectedRequest.responseBody.substring(0, 5000) + '\n\n... [truncated, ' + selectedRequest.responseBody.length + ' chars total]'
                          : selectedRequest.responseBody}
                      </div>
                    </div>
                  )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1, color: '#666' }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '32px', marginBottom: '12px' }}>🔍</div>
              <div style={{ fontSize: '14px', fontWeight: '600', marginBottom: '4px', color: '#888' }}>Select a request</div>
