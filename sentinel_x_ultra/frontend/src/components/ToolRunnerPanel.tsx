...
export function ToolRunnerPanel() {
  ...
  const handleStart = async () => {
    ...
    await ToolRunnerService.startTool(selectedTool, {
      ...params,
      target: params.target || 'example.com',
    });
    setShowNewRun(false);
    setParams({});
  };
      ...
      <div>
            ...
            {tools.map((t) => (
              ...
              >
                <div style={{ fontSize: '20px', marginBottom: '4px' }}>
                  {toolCategoryIcons[t.category] || '🔧'}
                </div>
                <div
                  style={{
                    fontSize: '11px',
                    fontWeight: '600',
                    color: t.installed ? '#00ff88' : '#ff4444',
                  }}
                >
                  {t.displayName}
                </div>
        ...
        {showNewRun && (
          ...
          >
            ...
            >
              <h4 style={{ fontSize: '14px', fontWeight: '600', color: '#00d4ff' }}>
                New Tool Run
              </h4>
              <button
        ...
        >
          ...
          >
            ...
            {runs.length > 0 && (
              <button
                onClick={() => ToolRunnerService.clearAll()}
                style={{
                  padding: '4px 10px',
                ...
                <div key={run.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                  ...
                  >
                    <div style={{ fontSize: '16px' }}>
                      {
                        toolCategoryIcons[
                          tools.find((t) => t.name === run.toolName)?.category || 'utility'
                        ]
                      }
                    </div>
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      {(run.state === 'running' || run.state === 'queued') && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            ToolRunnerService.cancelRun(run.id);
                          }}
                          style={{
                            padding: '4px 8px',
                      ...
                      {(run.state === 'completed' ||
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            ToolRunnerService.restartRun(run.id);
                          }}
                          style={{
                            padding: '4px 8px',
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          ToolRunnerService.clearRun(run.id);
                        }}
                        style={{
                          padding: '4px 8px',
