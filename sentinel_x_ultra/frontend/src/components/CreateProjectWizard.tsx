...
interface WizardProps {
  defaultName?: string
  onComplete: (name: string, description: string, folder: string, projectType: 'bug-bounty' | 'full-assessment') => void
  onCancel: () => void
}
...
export function CreateProjectWizard({ defaultName, onComplete, onCancel }: WizardProps) {
  ...
  const handleBrowse = async () => {
    ...
    try {
      ...
      const supportedExt = new Set(['.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.go', '.rb', '.php', '.sql', '.cs', '.c', '.cpp', '.h', '.hpp', '.yaml', '.yml', '.json', '.xml', '.html', '.css', '.txt', '.md', '.env', '.conf', '.ini'])
      const skipDirs = new Set(['node_modules', '.git', '__pycache__', 'venv', '.venv', 'dist', 'build', '.idea', '.vscode'])
  ...
  const getEnabledTabs = () => {
    ...
    return ['overview', 'recon', 'assets', 'findings', 'reports', 'proxy', 'intelligence', 'analysis']
  }
      ...
      <div style={{ animation: 'fadeInUp 0.3s ease' }}>
          ...
          {(['location', 'type', 'summary'] as const).map((s: Step, i) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{
                ...
                background: (step as string) === s ? 'linear-gradient(135deg, #00d4ff, #00ff88)' : s === 'location' ? '#00d4ff' : 'rgba(255,255,255,0.1)',
              ...
              <span style={{ fontSize: '12px', color: step === s ? '#00d4ff' : '#666', fontWeight: step === s ? '600' : '400' }}>
                {s === 'location' ? 'Location' : s === 'type' ? 'Type' : 'Summary'}
              </span>
              {i < 2 && <div style={{ width: '40px', height: '2px', background: step !== 'location' ? '#00d4ff' : 'rgba(255,255,255,0.1)' }} />}
            </div>
          ))}
        </div>

        <h2 style={{ fontSize: '22px', fontWeight: 'bold', marginBottom: '8px', textAlign: 'center' }}>Create New Project</h2>
        ...
        <div style={{ maxWidth: '480px', margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: '28px' }}>
            ...
            {folder && (
              <div style={{ marginTop: '8px', fontSize: '12px', color: '#00ff88', fontFamily: 'monospace' }}>
                📁 {folder}
              </div>
            )}
            {folder && !folder.startsWith('/') && !folder.includes(':/') && !folder.includes(':\\') && (
              <div style={{ marginTop: '6px', fontSize: '11px', color: '#ff8844', padding: '6px 10px', background: 'rgba(255,136,68,0.1)', borderRadius: '6px', border: '1px solid rgba(255,136,68,0.2)' }}>
                ⚠️ Replace the folder name above with the <strong>full path</strong> (e.g., <code style={{ color: '#ffaa00' }}>C:/Users/.../{folder}</code>) so the server can find your files
              </div>
            )}
            {folder && (folder.startsWith('/') || folder.includes(':/') || folder.includes(':\\')) && (
              <div style={{ marginTop: '6px', fontSize: '11px', color: '#00ff88', padding: '6px 10px', background: 'rgba(0,255,136,0.1)', borderRadius: '6px', border: '1px solid rgba(0,255,136,0.2)' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                padding: '14px 28px',
                background: isValidStep1 ? 'linear-gradient(135deg, #00d4ff, #00ff88)' : 'rgba(255,255,255,0.1)',
                border: 'none',
                borderRadius: '10px',
  ...
  if (step === 'type') {
    return (
      <div style={{ animation: 'fadeInUp 0.3s ease' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '32px', justifyContent: 'center' }}>
          {(['location', 'type', 'summary'] as Step[]).map((s) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{
                width: '32px', height: '32px', borderRadius: '50%',
                background: s === 'type' ? 'linear-gradient(135deg, #00d4ff, #00ff88)' : 'rgba(255,255,255,0.1)',
        ...
        <h2 style={{ fontSize: '22px', fontWeight: 'bold', marginBottom: '8px', textAlign: 'center' }}>Project Type</h2>
        <p style={{ fontSize: '13px', color: '#888', textAlign: 'center', marginBottom: '32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', maxWidth: '700px', margin: '0 auto' }}>
          ...
          <div
            ...
            style={{
              background: projectType === 'bug-bounty'
                ? 'rgba(170, 136, 255, 0.15)'
                : 'rgba(15, 15, 26, 0.95)',
              border: projectType === 'bug-bounty'
            ...
            onMouseEnter={(e) => { if (projectType !== 'bug-bounty') e.currentTarget.style.borderColor = 'rgba(170,136,255,0.3)' }}
            onMouseLeave={(e) => { if (projectType !== 'bug-bounty') e.currentTarget.style.borderColor = 'rgba(255,255,255,0.05)' }}
          >
            <div style={{ fontSize: '48px', marginBottom: '16px', textAlign: 'center' }}>🏴</div>
            <h3 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '8px', textAlign: 'center', color: projectType === 'bug-bounty' ? '#aa88ff' : '#fff' }}>
              Bug Bounty
            </h3>
          ...
          <div
            ...
            style={{
              background: projectType === 'full-assessment'
                ? 'rgba(0, 212, 255, 0.15)'
                : 'rgba(15, 15, 26, 0.95)',
              border: projectType === 'full-assessment'
            ...
            onMouseEnter={(e) => { if (projectType !== 'full-assessment') e.currentTarget.style.borderColor = 'rgba(0,212,255,0.3)' }}
            onMouseLeave={(e) => { if (projectType !== 'full-assessment') e.currentTarget.style.borderColor = 'rgba(255,255,255,0.05)' }}
          >
            <div style={{ fontSize: '48px', marginBottom: '16px', textAlign: 'center' }}>🛡️</div>
            <h3 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '8px', textAlign: 'center', color: projectType === 'full-assessment' ? '#00d4ff' : '#fff' }}>
              Full Assessment
            </h3>
            <p style={{ fontSize: '13px', color: '#888', textAlign: 'center', marginBottom: '16px', lineHeight: '1.5' }}>
        ...
        {/* Buttons */}
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'space-between', maxWidth: '700px', margin: '32px auto 0' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button
              ...
              style={{
                padding: '14px 28px',
                background: projectType ? 'linear-gradient(135deg, #00d4ff, #00ff88)' : 'rgba(255,255,255,0.1)',
                border: 'none',
                borderRadius: '10px',
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.3s ease' }}>
      <div style={{ maxWidth: '500px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '22px', fontWeight: 'bold', marginBottom: '8px', textAlign: 'center' }}>Review & Create</h2>
        <p style={{ fontSize: '13px', color: '#888', textAlign: 'center', marginBottom: '32px' }}>
        ...
        <div style={{
          ...
          <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ fontSize: '11px', color: '#666', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Project Name</div>
            <div style={{ fontSize: '16px', fontWeight: '600' }}>{name}</div>
          </div>
          {description && (
            <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ fontSize: '11px', color: '#666', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Description</div>
              <div style={{ fontSize: '14px', color: '#888' }}>{description}</div>
            </div>
          )}
          <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ fontSize: '11px', color: '#666', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Project Folder</div>
            <div style={{ fontSize: '13px', fontFamily: 'monospace', color: '#00ff88' }}>{folder}</div>
          </div>
          <div style={{ padding: '20px' }}>
            <div style={{ fontSize: '11px', color: '#666', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Project Type</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: '20px' }}>{projectType === 'bug-bounty' ? '🏴' : '🛡️'}</span>
              <span style={{ fontSize: '16px', fontWeight: '600', color: projectType === 'bug-bounty' ? '#aa88ff' : '#00d4ff' }}>
                {projectType === 'bug-bounty' ? 'Bug Bounty' : 'Full Assessment'}
              </span>
            </div>
            <div style={{ fontSize: '12px', color: '#888', marginTop: '12px' }}>
              Enabled tabs: {getEnabledTabs().map(t => t.charAt(0).toUpperCase() + t.slice(1)).join(', ')}
            </div>
          </div>
        </div>
