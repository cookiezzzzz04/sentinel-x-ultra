...
const DEFAULT_PIPELINE_STAGES: PipelineStage[] = [
  { id: 1, name: 'URL Parser', description: 'Parses HackerOne/BugCrowd URLs', status: 'waiting', icon: '🌐' },
  { id: 2, name: 'Policy Enforcer', description: 'Gatekeeper - reads program policy', status: 'waiting', icon: '🛡️' },
  { id: 3, name: 'Scope Guardian', description: 'Verifies authorized scope boundaries', status: 'waiting', icon: '🚮' },
  { id: 4, name: 'Passive Intel', description: 'Non-intrusive OSINT reconnaissance', status: 'waiting', icon: '🔍' },
  { id: 5, name: 'Active Enum', description: 'Direct interaction to map attack surface', status: 'waiting', icon: '🧰' },
  { id: 6, name: 'Vuln Scanner', description: 'OWASP Top 10 vulnerability testing', status: 'waiting', icon: '🔌' },
  { id: 7, name: 'Validation Engine', description: 'Multi-stage false positive elimination', status: 'waiting', icon: '✅' },
  { id: 8, name: 'Exploitation', description: 'Safe proof-of-concept creation', status: 'waiting', icon: '💥' },
  { id: 9, name: 'Analysis', description: 'CVSS scoring, CWE mapping', status: 'waiting', icon: '📊' },
  { id: 10, name: 'Report Gen', description: 'Professional Blank.md reports', status: 'waiting', icon: '📝' },
]
...
export function ExecutionMonitor({
  ...
  if (compact) {
    return renderDockedView({
      events,
      handleClear, handleExport, setSelectedEvent,
      onClose, isConnected,
    })
  }
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 280px', gap: '20px', height: 'calc(100vh - 120px)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', overflow: 'auto' }}>
        {/* Connection status */}
        <div style={{
          background: 'rgba(15,15,26,0.95)', borderRadius: '12px', padding: '16px',
          border: '1px solid rgba(255,255,255,0.05)',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
            <StatusIndicator status={isConnected ? 'online' : 'offline'} size="md" />
            <span style={{ fontSize: '13px', fontWeight: '600', color: isConnected ? '#00ff88' : '#ff4444' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '11px', color: '#888' }}>
            <div style={{ background: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '6px', textAlign: 'center' }}>
              <div style={{ fontSize: '18px', fontWeight: '700', color: '#00d4ff' }}>{status?.active_count || 0}</div>
              <div style={{ fontSize: '9px', color: '#666', marginTop: '2px' }}>ACTIVE</div>
            </div>
            <div style={{ background: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '6px', textAlign: 'center' }}>
              <div style={{ fontSize: '18px', fontWeight: '700', color: '#00ff88' }}>{status?.completed_count || 0}</div>
              <div style={{ fontSize: '9px', color: '#666', marginTop: '2px' }}>COMPLETED</div>
            </div>
            <div style={{ background: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '6px', textAlign: 'center' }}>
              <div style={{ fontSize: '18px', fontWeight: '700', color: '#ff4444' }}>{status?.failed_count || 0}</div>
              <div style={{ fontSize: '9px', color: '#666', marginTop: '2px' }}>FAILED</div>
            </div>
            <div style={{ background: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '6px', textAlign: 'center' }}>
              <div style={{ fontSize: '18px', fontWeight: '700', color: '#888' }}>{status?.total_events || 0}</div>
        ...
        <div style={{
          ...
          <h4 style={{ fontSize: '12px', fontWeight: '600', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '12px' }}>
            ⚡ Active Tasks
          </h4>
        ...
        {(status?.latest_failed || []).length > 0 && (
          <div style={{
            background: 'rgba(255,68,68,0.06)', borderRadius: '12px', padding: '14px',
            border: '1px solid rgba(255,68,68,0.15)',
          }}>
            <h4 style={{ fontSize: '12px', fontWeight: '600', color: '#ff4444', marginBottom: '10px' }}>
              ❌ Recent Failures
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {status?.latest_failed.map((f) => (
                <div key={f.id} style={{ fontSize: '11px', color: '#ff6666', padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px' }}>
                  {f.message}
                </div>
              ))}
            </div>
        ...
        {systemStatus && (
          <div style={{
            ...
            <h4 style={{ fontSize: '12px', fontWeight: '600', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px' }}>
              📊 Event Breakdown
            </h4>
            {Object.entries(systemStatus.event_type_breakdown).map(([type, count]) => (
              <div key={type} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', fontSize: '11px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1, marginLeft: '8px' }}>
                  ...
                  <span style={{ color: '#aaa', fontWeight: '600', minWidth: '20px', textAlign: 'right' }}>{count}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', overflow: 'auto' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '11px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: '#00d4ff', fontWeight: '600' }}>{projectId ? projectId.slice(0, 12) + '...' : 'Global'}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        {selectedEvent ? (
          ...
          <div style={{
            flex: 1,
            background: 'rgba(15,15,26,0.95)', borderRadius: '12px', padding: '20px',
            border: '1px solid rgba(255,255,255,0.05)',
        ...
        {onDockChange && (
          <div style={{
            background: 'rgba(15,15,26,0.95)', borderRadius: '12px', padding: '14px',
            border: '1px solid rgba(255,255,255,0.05)',
          }}>
            <h4 style={{ fontSize: '11px', fontWeight: '600', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px' }}>
              📌 Dock Position
            </h4>
              ...
              {dockPositions.map(dp => (
                <button
                  ...
                  style={{
                    padding: '8px 4px',
                    borderRadius: '6px',
                    border: dockPosition === dp.key ? '1px solid #00d4ff' : '1px solid rgba(255,255,255,0.08)',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }} />
        <button onClick={handleClear} style={dockBtnStyle}>🗑️</button>
        <button onClick={handleExport} style={dockBtnStyle}>📥</button>
        {onClose && <button onClick={onClose} style={dockBtnStyle}>✕</button>}
      </div>
...
function EventDetailCard({ event, onClose }: { event: LogEvent; onClose: () => void }) {
  const severityColors: Record<string, string> = {
    info: '#888', success: '#00ff88', warning: '#ffaa00', error: '#ff4444', debug: '#aa88ff',
  }
  return (
    <div style={{
      background: 'rgba(15,15,26,0.95)', borderRadius: '12px', padding: '16px',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        <h4 style={{ fontSize: '12px', fontWeight: '600', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          📋 Event Details
        </h4>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '11px' }}>
        <div style={{ padding: '10px', background: `${severityColors[event.severity] || '#888'}10`, borderRadius: '8px', borderLeft: `3px solid ${severityColors[event.severity] || '#888'}` }}>
          <div style={{ fontWeight: '600', color: '#fff', marginBottom: '4px' }}>{event.message}</div>
        ...
        {event.duration_ms !== undefined && <InfoRow label="Duration" value={`${event.duration_ms}ms`} />}
...
function InfoRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px' }}>
      <div style={{ color: '#666', fontSize: '9px', marginBottom: '2px', textTransform: 'uppercase' }}>{label}</div>
      <div style={{ color: color || '#ccc', fontWeight: '500', fontSize: '11px', wordBreak: 'break-all' }}>{value}</div>
    </div>
  )
}
...
function SystemStatusPanel({ status, events }: { status: SystemStatus | null; events: LogEvent[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {/* Health Card */}
      <div style={{
        background: 'rgba(15,15,26,0.95)', borderRadius: '16px', padding: '20px',
        border: '1px solid rgba(255,255,255,0.05)',
      }}>
        <h3 style={{ fontSize: '15px', fontWeight: '600', marginBottom: '16px' }}>📊 System Health</h3>
        ...
        {status?.active_descriptions && status.active_descriptions.length > 0 && (
          <div style={{ marginTop: '12px' }}>
            <div style={{ fontSize: '10px', color: '#666', textTransform: 'uppercase', marginBottom: '6px' }}>
              Currently Active
            </div>
            {status.active_descriptions.map((desc, i) => (
              <div key={i} style={{ fontSize: '11px', color: '#00d4ff', padding: '4px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                ▶ {desc}
              </div>
            ))}
          </div>
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: '15px', fontWeight: '600', marginBottom: '16px' }}>🔗 Source Activity</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {topSources.map(([source, count]) => {
            ...
            return (
              <div key={source} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '11px', color: '#aaa', fontWeight: '600', minWidth: '24px', textAlign: 'right' }}>{count}</span>
              </div>
            )
          })}
        </div>
      ...
      {status?.recent_errors && status.recent_errors.length > 0 && (
        <div style={{
          background: 'rgba(255,68,68,0.06)', borderRadius: '16px', padding: '20px',
          border: '1px solid rgba(255,68,68,0.15)',
        }}>
          <h3 style={{ fontSize: '15px', fontWeight: '600', color: '#ff4444', marginBottom: '12px' }}>
            ❌ Recent Errors ({status.recent_errors.length})
          </h3>
          {status.recent_errors.map((err) => (
            <div key={err.id} style={{
              padding: '10px', marginBottom: '8px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px',
              borderLeft: '3px solid #ff4444',
            }}>
              <div style={{ fontSize: '12px', color: '#ff6666', fontWeight: '600' }}>{err.message}</div>
...
function StatusMetric({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div style={{ textAlign: 'center', padding: '14px', background: 'rgba(0,0,0,0.2)', borderRadius: '10px' }}>
      <div style={{ fontSize: '24px', fontWeight: '700', color }}>{value}</div>
      <div style={{ fontSize: '10px', color: '#666', marginTop: '4px', textTransform: 'uppercase' }}>{label}</div>
    </div>
  )
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {Object.keys(groups).length === 0 ? (
          ...
          Object.entries(groups).reverse().map(([time, evts]) => (
                ...
                {evts.reverse().map(evt => (
                  <div key={evt.id} style={{
                    ...
                    borderLeft: `3px solid ${
                      evt.severity === 'error' ? '#ff4444' :
                      evt.severity === 'warning' ? '#ffaa00' :
                      evt.severity === 'success' ? '#00ff88' : '#888'
                    }`,
                    fontSize: '11px',
                  }}>
