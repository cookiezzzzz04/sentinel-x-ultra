...
const PROVIDERS: ProviderOption[] = [
  ...
  { id: 'lmstudio', name: 'LM Studio (Local)', defaultUrl: 'http://localhost:1234/v1', placeholder: 'not-required', models: ['llama3.1', 'local'] },
...
export function ModelManagementUI() {
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      const entries: ModelEntry[] = configuredProviders.map((provider: string, i: number) => ({
        ...
        provider,
        modelName: agentModels[provider] || PROVIDERS.find(p => p.id === provider)?.models[0] || 'auto',
  ...
  const testConnection = async (provider: string, apiKey: string, baseUrl: string, modelName: string) => {
    const testId = `test-${provider}-${Date.now()}`
    setTestingId(testId)
    setTestResults(prev => ({ ...prev, [testId]: { status: 'success', message: 'Testing...' } }))

    try {
      const res = await fetch(`/api/config/test?provider=${provider}&base_url=${encodeURIComponent(baseUrl)}`, {
        method: 'POST',
      ...
      if (data.status === 'ok') {
        setTestResults(prev => ({ ...prev, [testId]: { status: 'success', message: `✅ Connected! ${data.model || modelName} (${data.latency_ms}ms)` } }))
      } else {
        setTestResults(prev => ({ ...prev, [testId]: { status: 'error', message: `❌ ${data.error || 'Connection failed'}` } }))
      }
    } catch (e: any) {
      setTestResults(prev => ({ ...prev, [testId]: { status: 'error', message: `❌ ${e.message}` } }))
    } finally {
      setTestingId(null)
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div>
          <h2 style={{ fontSize: '22px', fontWeight: 'bold', marginBottom: '4px', color: '#00d4ff' }}>🤖 Model Management</h2>
          <p style={{ fontSize: '13px', color: '#888' }}>Configure and manage AI models for security analysis agents</p>
        </div>
        <Button
          variant="gradient"
          icon="+"
          onClick={() => { setShowAddModal(true); setFormProvider('openai'); setFormModelName('gpt-4o'); setFormApiEndpoint('https://api.openai.com/v1'); setFormApiKey(''); setFormEnabled(true); setFormError('') }}
        >
          Add Model
        </Button>
      </div>
      ...
      {/* Model Cards */}
      {models.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px 40px', background: 'rgba(15,15,26,0.95)', borderRadius: '16px', border: '1px dashed rgba(255,255,255,0.1)' }}>
          <div style={{ fontSize: '48px', marginBottom: '16px' }}>🤖</div>
          <h3 style={{ fontSize: '18px', marginBottom: '8px' }}>No models configured</h3>
          <p style={{ color: '#666', marginBottom: '24px' }}>Add an AI provider to enable security analysis agents</p>
          <Button variant="gradient" onClick={() => { setShowAddModal(true); setFormProvider('openai'); setFormModelName('gpt-4o'); }}>
              ...
              <div key={entry.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      <h4 style={{ fontSize: '15px', fontWeight: '600', color }}>{provider?.name || entry.provider}</h4>
                      <div style={{ fontSize: '12px', color: '#888', fontFamily: 'monospace' }}>{entry.modelName}</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <button
                      onClick={() => testConnection(entry.provider, entry.apiKey, entry.apiEndpoint, entry.modelName)}
                      style={{
                        padding: '6px 10px',
                        borderRadius: '6px',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <div>
                    <div style={{ fontSize: '10px', color: '#666', marginBottom: '2px' }}>Endpoint</div>
                    <div style={{ fontSize: '11px', color: '#aaa', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{entry.apiEndpoint}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '10px', color: '#666', marginBottom: '2px' }}>Status</div>
                    <StatusIndicator status={entry.enabled ? 'online' : 'offline'} label={entry.enabled ? 'Enabled' : 'Disabled'} size="sm" />
                  </div>
                  <div>
                    <div style={{ fontSize: '10px', color: '#666', marginBottom: '2px' }}>Temperature</div>
                    <div style={{ fontSize: '12px', color: '#888' }}>{entry.temperature}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '10px', color: '#666', marginBottom: '2px' }}>Context</div>
                    <div style={{ fontSize: '12px', color: '#888' }}>{entry.contextLength.toLocaleString()} tokens</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '10px', color: '#666', marginBottom: '2px' }}>Priority</div>
                ...
                {testResult && (
                  <div style={{
                    marginTop: '12px',
                    padding: '8px 12px',
                    background: testResult.status === 'success' ? 'rgba(0,255,136,0.1)' : 'rgba(255,68,68,0.1)',
                    borderRadius: '6px',
                    fontSize: '11px',
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {/* Provider */}
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px', fontWeight: '600' }}>Provider</label>
            <select
              value={formProvider}
          ...
          {/* Model Name */}
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px', fontWeight: '600' }}>Model Name</label>
            <input
              type="text"
              value={formModelName}
              ...
              style={{
                width: '100%', padding: '10px', background: 'rgba(0,0,0,0.3)',
                border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#00d4ff', fontSize: '13px', fontFamily: 'monospace',
              }}
              placeholder="gpt-4o"
            />
          </div>
          ...
          {/* API Endpoint */}
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px', fontWeight: '600' }}>API Endpoint</label>
            <input
              type="text"
              ...
              style={{
                width: '100%', padding: '10px', background: 'rgba(0,0,0,0.3)',
                border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', fontSize: '13px', fontFamily: 'monospace',
              }}
          ...
          {/* API Key */}
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px', fontWeight: '600' }}>API Key</label>
            <input
              type="password"
              value={formApiKey}
              ...
              placeholder={PROVIDERS.find(p => p.id === formProvider)?.placeholder || 'Enter API key'}
            />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px', fontWeight: '600' }}>Temperature</label>
              <input
                type="number"
                ...
                style={{
                  width: '100%', padding: '10px', background: 'rgba(0,0,0,0.3)',
                  border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#ffaa00', fontSize: '13px',
                }}
              />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px', fontWeight: '600' }}>Context Length</label>
              <input
                type="number"
                ...
                style={{
                  width: '100%', padding: '10px', background: 'rgba(0,0,0,0.3)',
                  border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#00d4ff', fontSize: '13px',
                }}
              />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', alignItems: 'end' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px', fontWeight: '600' }}>Priority</label>
              <input
                type="number"
                value={formPriority}
          ...
          {/* Error */}
          {formError && (
            <div style={{ padding: '10px', background: 'rgba(255,68,68,0.1)', borderRadius: '8px', fontSize: '12px', color: '#ff4444' }}>
              {formError}
            </div>
          )}
